...
aToB:						# void aToB(const int *a, size_t size, int *result)
	...
	mov	QWORD PTR -32[rbp], rsi		# size_t size
	mov	QWORD PTR -40[rbp], rdx		# int *result
	mov	DWORD PTR -12[rbp], 1		# flag = 1
...
.L6:
	...
	mov	DWORD PTR -12[rbp], 0		# flag = 0
.L3:
	cmp	DWORD PTR -12[rbp], 0
	je	.L4				# flag == 0 ?
	mov	rax, QWORD PTR -8[rbp]		# if (flag)
	lea	rdx, 0[0+rax*4]
	mov	rax, QWORD PTR -24[rbp]
	add	rax, rdx
...
.L2:
	mov	rax, QWORD PTR -8[rbp]		
	cmp	rax, QWORD PTR -32[rbp]		# for (size_t i = 0; i < size; i++)
	jb	.L6
	nop
	nop
	pop	rbp
